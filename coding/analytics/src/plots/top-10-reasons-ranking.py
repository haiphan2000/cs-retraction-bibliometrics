...
def render_retraction_reason_chart(
    df: pd.DataFrame,
    ...
    max_line_length = 28
    y_labels = df_plot["reason"].tolist()
    y_labels = [
    ...
    fig, ax = plt.subplots(figsize=(CHART_WIDTH, 5.50))
    ...
    fig.text(0, 1, "REASON", fontsize=FOOTNOTE_FONT_SIZE, color=palette["GRAY_DARK"], ha="right")
    ...
    fig.text(0.365, 1.005, "|", fontsize=FOOTNOTE_FONT_SIZE, color=palette["GRAY_DARK"], ha="center")
    fig.text(
    0.385,
    1,
    ...
    file_path = "output/top-10-subjects-ranking.png"
    fig.savefig(
        file_path,
        bbox_inches="tight",
    )
    ...
    if plot_area_width > 0 and "TARGET_IMAGE_WIDTH" in globals():
        ...
        if plot_area_width > 0:
            ...
            fig.set_size_inches(new_chart_width, 5.50)
            fig.canvas.draw()
