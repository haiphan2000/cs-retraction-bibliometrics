...
def render_retraction_journal_chart(
    df: pd.DataFrame,
    ...
    max_line_length = 27
    y_labels = df_plot["journal"].tolist()
    y_labels = [
        "\n".join(textwrap.wrap(label, width=max_line_length))
    ...
    fig, ax = plt.subplots(figsize=(CHART_WIDTH, 3))
    ...
    n = len(total_vals)
    highlighted_visual_positions = {1}
    highlighted_indices = {
    ...
    fig.text(0, 1, "JOURNAL", fontsize=FOOTNOTE_FONT_SIZE, color=palette["GRAY_DARK"], ha="right")
    ...
    fig.text(0.36, 1.005, "|", fontsize=FOOTNOTE_FONT_SIZE, color=palette["GRAY_DARK"], ha="center")
    fig.text(
    0.375,
    1,
    ...
    file_path = "output/top-5-journals-ranking.png"
    fig.savefig(
        file_path,
        bbox_inches="tight",
    )
    ...
    if plot_area_width > 0 and "TARGET_IMAGE_WIDTH" in globals():
        ...
        if plot_area_width > 0:
            ...
            fig.set_size_inches(new_chart_width, 3.00)
            fig.canvas.draw()
